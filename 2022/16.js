...
function runBFS(start, curr, dist, visited) {
  ...
  while (i < queue.length) {
    ...
    if (visited[curr] != undefined) { continue; }
    visited[curr] = dist;
...
function getMostPressure(curr, timeLeft, opened, cache) {
  ...
  const res = Math.max(
    ...neighbors.map((neighbor) => {
      ...
      left = getMostPressure(neighbor, timeLeft - 1, opened, cache)
      // opening valve
      let right = 0
...
function getMostPressureElephant(
  ...
  if (eTimeLeft > timeLeft) {
    return getMostPressureElephant(elephant, curr, eTimeLeft, timeLeft, opened, cache)
  }
  ...
  if (otherCacheKey in cache) { return cache[otherCacheKey] }
  ...
  const left = Math.max(
    ...Object.keys(VALVES).map((neighbor) => {
      if (neighbor == elephant || neighbor == curr || neighbor == 'AA') { return 0; }
      if (opened.has(neighbor)) { return 0; }
  ...
  if (!opened.has(curr) && rate > 0) {
    opened.add(curr)
    right =
      getMostPressureElephant(curr, elephant, timeLeft - 1, eTimeLeft, opened, cache) +
      (timeLeft - 1) * rate
    opened.delete(curr)
  }
...
function getMostPressureE(curr, timeLeft, pathsLeft, opened, cache) {
  ...
  if (timeLeft <= 0 && pathsLeft > 0) return getMostPressureE('AA', 26, pathsLeft - 1, opened, cache)
  ...
  const left = Math.max(
    ...Object.keys(VALVES).map((neighbor) => {
      if (neighbor == curr || neighbor == 'AA') { return 0; }
      if (opened.has(neighbor)) { return 0; }
      let distance = DISTANCES[curr][neighbor];
...
async function getMaxRelease(file = '../input.txt') {
  ...
  const res = getMostPressureE('AA', 26, 1, opened, cache)
